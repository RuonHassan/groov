...
const REDIRECT_URI = Deno.env.get('REDIRECT_URI') || 'https://groov-tasks.com/auth/outlook/callback';
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[Edge Function] Using OUTLOOK_CLIENT_ID: ${OUTLOOK_CLIENT_ID?.substring(0, 10)}...`); // Log first 10 chars
    console.log(`[Edge Function] OUTLOOK_CLIENT_SECRET length: ${OUTLOOK_CLIENT_SECRET?.length}`); // Log length only
    ...
    console.log('[Edge Function] Sending token exchange request to Outlook...'); // Log before fetch
    ...
    if (!tokenResponse.ok) {
      console.error('[Edge Function] Token exchange error response from Outlook:', tokens); // Log the actual error object from Outlook
    ...
    if (!calendarResponse.ok) {
      console.error('[Edge Function] Calendar fetch error:', calendar);
